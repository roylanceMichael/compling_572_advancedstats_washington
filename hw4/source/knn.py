...
class KNN:
    ...
    def prepInput(self):
        ...
        for i in ilist[1:]:
            ...
            pair = i.split(':')
            if len(pair) == 2:
                self.testVectDict[pair[0]] = pair[1]
    ...
    def distanceEuclidean(self, trainInstance):
        ...
        for feature in self.allFeatures:
            if feature in self.testVectDict and feature in trainInstance:
                addSum += math.pow((float(trainInstance[feature]) - float(self.testVectDict[feature])), 2)
            elif feature in self.testVectDict:
                addSum += math.pow(float(self.testVectDict[feature]), 2)
            elif feature in trainInstance:
                addSum += math.pow(float(trainInstance[feature]), 2)
...
#    def distanceCosine(self):
    ...
    def knn(self):
        ...
        print self.doc
        for Id in self.tr.vectors:
            d = self.getDistance(self.tr.vectors[Id].features)
